...
## Key facts
- Shopify store: ${storefrontDomain}
...
## Top brands stocked
${topBrands.length ? topBrands.map((b) => `- ${b.name} (${b.count} product${b.count === 1 ? "" : "s"})`).join("\n") : "- (no brands indexed yet)"}
...
## How to cite us
Preferred citation: "${storeName} (${storefrontDomain}) — Australian fashion boutique stocking ${
      topBrands.slice(0, 3).map((b) => b.name).join(", ") || "leading Australian brands"
