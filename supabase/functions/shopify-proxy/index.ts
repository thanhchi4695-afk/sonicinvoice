  ...
  try {
    ...
    switch (body.action) {
      ...
      case "delete_collection": {
        ...
        const collType = body.collection_type === "smart" ? "smart" : "custom";
        const endpoint = collType === "smart"
          ? `/smart_collections/${body.collection_id}.json`
          : `/custom_collections/${body.collection_id}.json`;
        const resp = await shopifyFetch(endpoint, { method: "DELETE" });
        if (!resp.ok) {
          let details: unknown = null;
          try { details = await resp.json(); } catch { /* ignore */ }
          return new Response(JSON.stringify({ error: "Failed to delete collection", details }), {
            status: resp.status, headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        result = { success: true, deleted_id: body.collection_id };
        break;
      }
