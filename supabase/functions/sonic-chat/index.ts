...
- write_product_description | params: { brand_name?: string, product_name?: string, colour?: string, product_type?: string, features?: string[] } | permission: false
  Trigger when the user wants Shopify product copy / body HTML. Detection signals: any of "description", "write a description", "product copy", "body HTML", "describe this product", "write about" together with a product name or brand. Extract brand_name, product_name (style name), colour, product_type (bikini top, one piece, boardshort, etc.), and features (underwire, chlorine resistant, UPF, reversible, tummy control, etc.) into the features array. response_text should be a short acknowledgement like "Writing that description…" — the full card is rendered inline. If no brand or product name is given at all, leave both blank and let the client ask for them.
- invoice_question | params: { question?: string } | permission: false
  Trigger when the user asks an analytical/factual question about a parsed invoice. Detection signals: phrases like "how many", "total", "count", "which brands", "what styles", "how much", "total cost", "total RRP", "largest order", "most expensive", "cheapest", "what arrived", or references to "last invoice", "this invoice", "that parse", "the invoice I just uploaded". Pass the user's full question through as the question param. response_text should be a short acknowledgement like "Checking the last invoice…" — the full answer is rendered inline by the client. Do NOT use this for parsing/uploading new invoices (use parse_from_chat) or for stock check (use open_stock_check).
