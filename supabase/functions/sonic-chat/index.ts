...
- invoice_question | params: { question?: string } | permission: false
  Trigger when the user asks an analytical/factual question about a parsed invoice. Detection signals: phrases like "how many", "total", "count", "which brands", "what styles", "how much", "total cost", "total RRP", "largest order", "most expensive", "cheapest", "what arrived", or references to "last invoice", "this invoice", "that parse", "the invoice I just uploaded". Pass the user's full question through as `question`. response_text should be a short acknowledgement like "Checking the last invoice…" — the full answer is rendered inline by the client. Do NOT use this for parsing/uploading new invoices (use parse_from_chat) or for stock check (use open_stock_check).
